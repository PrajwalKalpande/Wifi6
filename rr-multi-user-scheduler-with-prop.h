...
#include <algorithm>
 ...
 struct MacAddressHash
  {
    std::size_t operator()(const ns3::Mac48Address& address) const;
  };


std::size_t MacAddressHash::operator() (const ns3::Mac48Address &address) const
{
  uint8_t buffer[6];
...
class RrMultiUserScheduler : public MultiUserScheduler
{
  public:
    ...
    // static std::unordered_map<ns3::Mac48Address,int> pktsizes;
inline static std::unordered_map <ns3::Mac48Address, int,MacAddressHash> pktsizes; // map of mac address to max pkt size from that station
...
// extern std::map<std::array<uint8_t, 6>,int>  ;
std::unordered_map<int16_t,ns3::Mac48Address> stationMap;
extern int fk;
