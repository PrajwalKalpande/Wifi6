...
using namespace ns3;
// using ::operator<<;
// std::unordered_map <Mac48Address, int, MacAddressHash> pktsizes;
...
public:
  ...
  void NotifyPhyReceived(Ptr<const Packet> p);
...
std::cout<<"Done\n";
 ...
 dev->GetPhy ()->TraceConnectWithoutContext ("PhyRxEnd", MakeCallback (&WifiOfdmaExample::NotifyPhyReceived, this));
...
void  WifiOfdmaExample::NotifyPhyReceived(Ptr<const Packet> p){
  std::cout<<"Packet size at Phy level "<<p->GetSize()<<"\n";
  return;
}
void 
...
// pktsizes[]
// if(listIt->m_dstAddress== apAddress)
  ...
  if (MacAddressToNodeId (listIt->m_srcAddress) == 0)
  ...
  else
    {
      ...
      vecIt = std::next (m_ulPerStaAcStats.begin (), MacAddressToNodeId (listIt->m_srcAddress) - 1);
       ns3::RrMultiUserScheduler:: pktsizes[listIt->m_srcAddress] =std::fmax(p->GetSize() ,ns3::RrMultiUserScheduler:: pktsizes[listIt->m_srcAddress]);
    
    std::cout<< "updated size "<<ns3::RrMultiUserScheduler::  pktsizes[listIt->m_srcAddress]<<" "<<  p->GetSize()  <<" "<<listIt->m_srcAddress<<"\n";

      //pktsizes[listIt->m_srcAddress] =std:f:max(p->GetSize() ,pktsizes[listIt->m_srcAddress]);
